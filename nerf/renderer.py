...
class NeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, num_steps=128, upsample_steps=128, 
        ...
        # reshaping to (B*N, 3) where B is batch size and N is number of rays in a batch
        ...
        print("fars and nears in run cuda: ", fars, nears)
        #print(torch.max(fars))
        #print(torch.min(fars))
        #print(torch.max(nears))
        #print(torch.min(nears))
        ...
        scene volume defined by aabb.
        '''
        # generate xyzs
        xyzs = rays_o.unsqueeze(-2) + rays_d.unsqueeze(-2) * z_vals.unsqueeze(-1) # [N, 1, 3] * [N, T, 1] -> [N, T, 3]
        xyzs = torch.min(torch.max(xyzs, aabb[:3]), aabb[3:]) # a manual clip. #[N, T, 3]

        print("Shape of points along the ray: ", xyzs.shape) #[N, T, 3]
        ...
        print("Density outputs shape: ", len(density_outputs))
        # print("Type of density outputs: ", len(density_outputs[0]))
        print("Density outputs: ")
        ...
        print("SHape of dirs: ", dirs.shape)
        ...
        # print("Printing self.collor net......")
        # print(self.color_net)

        # print("="*50)
        # for name, l in self.color_net.named_modules():
        #     print(l, type(l), name)
        #     if isinstance(l, nn.BatchNorm1d):
        #         print("Hurray!!")
        #     if isinstance(l, nn.Linear):
        #         print("Detected linear")

        # print("Exiting system from the rendere function....")
        #sys.exit()

        
        rgbs = self.color(xyzs.reshape(-1, 3), dirs.reshape(-1, 3), mask=mask.reshape(-1), **density_outputs) # [N, T, 3]
        ...
        image = image.view(*prefix, 3)  # shape [1, 4096, 3]
        depth = depth.view(*prefix)  # shape [1, 4096]

        print('Type image and depth: ', type(image), type(depth))
        print('Shape of image and depth: ', image.shape, depth.shape)
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, 
        ...
        if self.training:
            ...
            print("yes, inside self.training....")
            
            '''
            This line of code calls a function called march_rays_train from the raymarching module, which performs 
            ray marching to generate 3D points (xyzs), directions (dirs), delta distances (deltas), and 
            ray indices (rays) for each ray. The arguments include the original ray positions and 
            directions (rays_o and rays_d), various parameters for the
            '''
            ...
            # to show the predicted depth and rgb images
            cv2.imshow("Pred color img", image)
            cv2.waitKey(0)

            cv2.imshow("Pred depth image", depth)
            cv2.waitKey(0)
    ...
    def render(self, rays_o, rays_d, staged=False, max_ray_batch=4096, 
        ...
        if self.cuda_ray:
            _run = self.run_cuda
            print("Running cuda ray....")
        else:
            print("Not running cuda ray...")
            _run = self.run
        ...
        if staged and not self.cuda_ray:
            print("Inisde if statement in render function...")
            ...
            print("Inside else statement in the render function....")
