...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None, camera_model='pinhole', patch_size=1):
    ...
    if camera_model == "touch":
        #print("USING THIS FX")
        #print(fx)
        # to get the fov for the touch sensor by making use of the sensor size
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        if 'images' not in data:
            ...
            print("Dont't have images in the data")
        ...
        print("Value of B, N, and C are as follows: ", B, N, C)
        ...
        if C == 3 or self.model.bg_radius > 0:
            bg_color = 1  # to set the background to be of the white color
        ...
        if C == 4:
            '''
            If the input images have an alpha channel (C=4) and the previous condition is not met, 
            the code generates the ground truth RGB color by compositing the RGB values of the 
            image with its alpha channel and the background color. Specifically, it computes the 
            weighted sum of the image RGB values and alpha channel, and the weighted sum of the 
            background color and (1 - alpha channel). The resulting ground truth RGB color is 
            assigned to 'gt_rgb'.
            '''
        ...
        #print(data['near'])
        #print(" ")
        print("Visualizing the groud truth rgb values....")
        print("Ground truth rgb: ", gt_rgb.shape)

        # tensor_np = gt_rgb.squeeze().cpu().numpy()  # Convert tensor to numpy array and remove the first dimension
        # tensor_np = np.reshape(tensor_np, [64, 64, 3])  # Reshape the array to [64, 64, 3]
        # color_indicator = tensor_np[:, :, -1]  # Extract the last channel of the array

        # print("Color indicator shape: ", color_indicator.shape)
        # print("Values of the color indicator: ",color_indicator)

        # tensor_np = np.uint8(tensor_np * 255)
        # image = Image.fromarray(tensor_np, 'RGB')

        # print('Saved the gt image.....')

        # image.save("/home/tejas/Documents/Stanford/ARMLab/Nerf-By-Touch/torch-ngp/gt_image.png")
        ...
        if self.error_map[data['type']] is not None:
            print("I am updating ERROR MAP TOO!!!!!!")
            index = data['index'] # [B]
        ...
        #loss = loss.mean()
        #print("Final; loss to be returned: ", loss)
    ...
    def train_gui(self, train_loader, step=16):
        ...
        print("Length of total loss: ", len(total_loss))
       

        # allow for multiple datasets to be trained at once!
        # ensure only the smaller datasets are cycled through!
        loader_lens = [len(train_loader[i]) for i in range(len(train_loader))]
        print("Loader Lens value: ", loader_lens)
        index = loader_lens.index(max(loader_lens))
        ...
        for _ in range(step):
            ...
            # print(loader)
            # taking a single image from each dataset as input along with the intrinsic parameters
            data = next(loader)
            print()
            print("DATA:")
            print(data)
            #print('Length of data: ', len(data), len(data[0]), len(data[1]))
            print()
            print("Lengths of images: ", len(data[0]["images"]))
            #print("Exiting")
            #sys.exit()
            # print("HMMMMMMM")
            # print(data)
            ...
            # taking 1 type of image from the data at a time
            for d in data:
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    ...
                    print("Type and shape of preds: ", type(preds), preds.shape)
                    print("Type and shape of gt: ", type(truths), truths.shape)

                    # to visualize the truths
                    gt = preds
                    ...
                    # fig = plt.figure()
                    # ax = fig.add_subplot(projection='3d')
                    # ax.scatter(gt[:, 0], gt[:, 1], gt[:, 2], s=5, c=gt[:,::-1], marker='o')
