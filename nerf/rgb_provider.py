...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        if self.mode == 'colmap' and type == 'test':
            ...
            for f in tqdm.tqdm(frames, desc=f'Loading {type} data'):
                ...
                if "cameras" in transform:
                    ...
                    #print("Have multiple cameras in use....")
                ...
                # only one camera in use
                    ...
                    #print("Have only 1 camera in use.....")
                ...
                if image.shape[-1] == 3: 
                    ...
                    #print("Don't have an alpha channel....")
                else:
                    image = cv2.cvtColor(image, cv2.COLOR_BGRA2RGBA)
                    #print("Have an alpha channel.....")
        ...
        if self.training and self.opt.error_map:
            ...
            print("Error map is not provided....")
            self.error_map = None

        # [debug] uncomment to view all training poses.
        print("LENGTH: ", len(self.poses))
        #print('Exiting....')
        #sys.exit()
        ...
        print("Printing the intrinsics: ", self.intrinsics)
        print()
        # print("intrinics")
        ...
        # print()
        # print("Length: ", len(self.intrinsics))
        # print()
    ...
    def collate(self, index):
        ...
        if self.rand_pose == 0 or index >= len(self.poses):
            ...
            print("Value of s: ", s)
            print("Values of rH and rW: ", rH, rW)

            print("Intrinsics: ", self.intrinsics[index])
        ...
        if self.images is not None:
            ...
            print("Image original shape: ", images.shape)

            '''
            If the RGBProvider is being used for training, the code gathers the image colors for each
            ray using the 'torch.gather()' function. Specifically, it first reshapes the images tensor 
            to [B, H*W, C], where C is the number of color channels, using the '.view()' method. Then, 
            it stacks 'C' copies of the 'rays['inds']' tensor (which contains the pixel indices for 
            each ray) along the last dimension using 'torch.stack()', resulting in a tensor of shape 
            [B, N, C], where N is the number of samples along each ray. Finally, 'torch.gather()' is 
            called with the reshaped images tensor and the stacked indices tensor to gather the colors 
            of the pixels corresponding to each sample along each ray. The resulting tensor has shape 
            [B, N, C].
            '''
            ...
            if self.training:
                ...
                print("After shooting rays shape of images: ", images.shape)
                #sys.exit()
    ...
    def dataloader(self):
        ...
        print()
        print("Loader: ", loader)
        #print("_data: ", loader._data)
        #print("Has gt: ", loader.has_gt)
        #sys.exit()
